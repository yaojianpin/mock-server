...
impl DataConfig {
  ...
  pub fn wrapping_page(&self, items: Vec<&Value>, total: usize, page: usize, size: usize) -> Value {
  ...
  fn routing_parse(&mut self, data: &Value) {
    ...
    if !routing_data.is_null() {
      self.routing = serde_json::from_value::<HashMap<String, RoutingValue>>(routing_data).unwrap();
    }
  }
